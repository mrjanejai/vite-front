      ...
      <div class="col">
        <div class="col-12"><input id="searchbar_toggle" type="checkbox" />
          <div id="searchbar" class="mb-4">
            <div class="form-group col-lg-2 me-2 mb-lg-0 mb-3">
              <select id="search_col" @change="searchChange()" class="form-select form-select-sm">
                <option value="MaType.id" data-type="number">Ma Type Id</option>
          ...
          <table class="table table-sm table-striped table-hover">
            <thead>
              <tr>
                <th :class="getSortClass('MaType.id','asc')">
                  <router-link :to="getLink('sort','MaType.id','asc')">Id</router-link>
                </th>
                <th :class="getSortClass('MaType.sla_name')">
                  <router-link :to="getLink('sort','MaType.sla_name')">Sla Name</router-link>
                </th>
                <th>Actions</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="maType in maTypes" :key="maType">
                <td class="text-center">{{maType.id}}</td>
                <td>{{maType.sla_name}}</td>
                <td class="text-center">
                  <router-link class="btn btn-sm btn-secondary" :to="`/maType/${maType.id}`" title="View"><i class="fa fa-eye"></i></router-link>
                  <router-link class="btn btn-sm btn-primary" :to="`/maType/edit/${maType.id}`" title="Edit"><i class="fa fa-pencil"></i></router-link>
                  <router-link class="btn btn-sm btn-danger" :to="`/maType/delete/${maType.id}`" title="Delete"><i class="fa fa-times"></i></router-link>
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <router-link class="btn btn-sm btn-primary" to="/maType/create">Create</router-link>
        </div>
